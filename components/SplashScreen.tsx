...
// 타임라인: 0~0.6s 원 그리기 → 0.45~0.95s 로고 페이드인(겹침) → ~1.3s까지 유지 → 0.35s 슬라이드아웃(왼쪽으로)
const HOLD_MS = 1300;
const EXIT_MS = 350;
...
export default function SplashScreen() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => setPhase('hidden'), HOLD_MS + EXIT_MS);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: phase === 'fading' ? 'translateX(-100%)' : 'translateX(0)',
